...
drop function if exists public.translation_workspace_session(text, text, integer, integer);
...
as $$
    ...
    where tk.status = 'active'
      ...
      and (
        ...
        or exists (
          ...
          where lower(trim(target_code.value)) = lower(st.code)
        )
      )
      and not exists (
        select 1
  ...
  batch_keys as (
    select
      ...
      ck.subcategory,
      ck.context,
      ck.protected_variables,
      ck.protected_terms,
      ck.source_language_id,
      ck.source_language_code,
      ck.source_language_name,
      ck.source_language_native_name,
      ck.source_language_emoji,
      ck.target_language_id,
      ck.target_language_code,
      ck.target_language_name,
      ck.target_language_native_name,
      ck.target_language_emoji,
      ck.queue_rank
    from candidate_keys ck
    cross join normalized_params np
    order by ck.queue_rank asc
  ...
  select
    gen_random_uuid()::uuid as session_id,
    st.id::uuid as target_language_id,
    st.code::text as target_language_code,
    st.name::text as target_language_name,
    st.native_name::text as target_language_native_name,
    st.emoji::text as target_language_emoji,
    np.normalized_category_slug::text as category_slug,
    coalesce(t.total_available, 0::bigint)::bigint as total_available,
    count(ir.translation_key_id)::bigint as loaded_count,
    np.normalized_batch_size::integer as batch_size,
    np.normalized_session_offset::integer as session_offset,
    (coalesce(t.total_available, 0::bigint) > (np.normalized_session_offset::bigint + count(ir.translation_key_id)::bigint)) as has_more,
    coalesce(
      jsonb_agg(
        jsonb_build_object(
