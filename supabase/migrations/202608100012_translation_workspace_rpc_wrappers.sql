...
drop function if exists public.translation_workspace_detect_duplicate(uuid, text, text);

create function public.translation_workspace_detect_duplicate_impl(
...
create function public.translation_workspace_detect_duplicate(
...
as $$
  select *
  from public.translation_workspace_detect_duplicate_impl(
    translation_key_id,
...
create function public.translation_workspace_submit_impl(
  _translation_key_id uuid,
  _target_language_code text,
  _suggestion_text text,
...
as $$
  select *
  from public.translation_workspace_submit_impl(
    translation_key_id,
    target_language_code,
    suggestion_text,
    rationale,
